...
import { Save, Plus, Package, Wrench } from "lucide-react";
...
export default function AddItemDialog({ 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (onItemAdded) {
        onItemAdded(result.data || result);
      }
  ...
  const handleAddUnit = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        loadUnits();
        setFormData(prev => ({ ...prev, unit_code: unit.code }));
  ...
  // Don't render if FPO ID is not available
  if (!fpoId) {
    return null;
  }
