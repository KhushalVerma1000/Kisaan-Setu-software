...
import { updateJournalVoucherAction } from "@/server/features/vouchers/infrastructure/api/actions/voucherActions";
...
export default function EditJournalVoucherPage() {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          if (voucher.notes && voucher.notes.startsWith('Journal Type: ')) {
            const type = voucher.notes.replace('Journal Type: ', '').toLowerCase();
            if (['adjustment', 'correction', 'transfer', 'accrual', 'provision', 'other'].includes(type)) {
              setJournalType(type as any);
            }
          }
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      // Call server action for updating journal voucher
      const response = await updateJournalVoucherAction({
        voucherId,
      ...
      if (response.success) {
        toast.success(response.data?.message || "Journal voucher updated successfully");
        router.push("/Journal");
      } else {
        toast.error(response.error || "Failed to update journal voucher");
      }
    } catch (error) {
      console.error('Submit error:', error);
      toast.error(error instanceof Error ? error.message : "Failed to update journal voucher");
    } finally {
      setIsSubmitting(false);
    }
  }, [validateForm, voucherId, narration, journalType, entries, router]);
      ...
      <div className="max-w-7xl mx-auto space-y-4 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Edit Journal Voucher</h1>
