...
import { updateReceiptVoucherAction } from "@/server/features/vouchers/infrastructure/api/actions/voucherActions";
...
export default function EditReceiptVoucherPage() {
    ...
    const fetchVoucherData = async () => {
      ...
      try {
        ...
        if (result.success && result.data) {
          ...
          // Parse ledger entries into cash/bank account and receipt entries
          ...
          setReceiptData({ cashBankAccount, receiptEntries });
        } else {
  ...
  const handleSubmit = useCallback(async () => {
    ...
    try {
      ...
      const response = await updateReceiptVoucherAction({
        voucherId,
      ...
      if (response.success) {
        toast.success(response.data?.message || "Receipt voucher updated successfully");
        router.push("/PaymentReceiptContra");
      } else {
        toast.error(response.error || "Failed to update receipt voucher");
      }
    } catch (error) {
      console.error('Submit error:', error);
      toast.error(error instanceof Error ? error.message : "Failed to update receipt voucher");
    } finally {
      setIsSubmitting(false);
    }
  }, [validateForm, voucherId, narration, receiptData, totalAmount, router]);
      ...
      <div className="max-w-7xl mx-auto space-y-4 p-4 sm:p-6 lg:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Edit Receipt Voucher</h1>
